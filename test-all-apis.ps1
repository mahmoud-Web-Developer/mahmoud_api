...
# 12. اختبار لوحة التحكم (مع Token)
if ($token) {
    Write-Host "12. اختبار لوحة التحكم..." -ForegroundColor White
    try {
        ...
        Write-Host "   Status: $($dashboardResponse.success)" -ForegroundColor Gray
    } catch {
        Write-Host "❌ فشل في الوصول للوحة التحكم" -ForegroundColor Red
        Write-Host "   الخطأ: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "12. اختبار لوحة التحكم..." -ForegroundColor Yellow
    Write-Host "⚠️ لا يمكن اختبار لوحة التحكم بدون Token" -ForegroundColor Yellow
}

Write-Host ""

# 13. اختبار إضافة عميل (مع Token)
if ($token) {
    Write-Host "13. اختبار إضافة عميل..." -ForegroundColor White
    try {
        $headers = @{ "Authorization" = "Bearer $token" }
        $clientData = @{
            name = "Test Client"
        ...
        $clientResponse = Invoke-RestMethod -Uri "$baseUrl/clients" -Method POST -Body $clientData -Headers $headers -ContentType "application/json" -TimeoutSec 10
        Write-Host "✅ إضافة عميل:" -ForegroundColor Green
        Write-Host "   Status: $($clientResponse.success)" -ForegroundColor Gray
    } catch {
        Write-Host "❌ فشل في إضافة عميل" -ForegroundColor Red
        Write-Host "   الخطأ: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "13. اختبار إضافة عميل..." -ForegroundColor Yellow
    Write-Host "⚠️ لا يمكن اختبار إضافة عميل بدون Token" -ForegroundColor Yellow
}

Write-Host ""

# 14. اختبار جلب العملاء (مع Token)
if ($token) {
    Write-Host "14. اختبار جلب العملاء..." -ForegroundColor White
    try {
        $headers = @{ "Authorization" = "Bearer $token" }
        $clientsResponse = Invoke-RestMethod -Uri "$baseUrl/clients" -Method GET -Headers $headers -TimeoutSec 10
        Write-Host "✅ جلب العملاء:" -ForegroundColor Green
        Write-Host "   عدد العملاء: $($clientsResponse.data.length)" -ForegroundColor Gray
    } catch {
        Write-Host "❌ فشل في جلب العملاء" -ForegroundColor Red
        Write-Host "   الخطأ: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "14. اختبار جلب العملاء..." -ForegroundColor Yellow
    Write-Host "⚠️ لا يمكن اختبار جلب العملاء بدون Token" -ForegroundColor Yellow
}

Write-Host ""

# 15. اختبار طلبات Admin (مع Token)
if ($token) {
    Write-Host "15. اختبار طلبات Admin..." -ForegroundColor White
    try {
        $headers = @{ "Authorization" = "Bearer $token" }
        ...
        $adminContactData = @{
            ...
            message = "Admin contact request"
        } | ConvertTo-Json
        
        $adminContactResponse = Invoke-RestMethod -Uri "$baseUrl/requests/contact" -Method POST -Body $adminContactData -Headers $headers -ContentType "application/json" -TimeoutSec 10
        Write-Host "✅ طلب تواصل Admin:" -ForegroundColor Green
        Write-Host "   Status: $($adminContactResponse.success)" -ForegroundColor Gray
    } catch {
        Write-Host "❌ فشل في طلب تواصل Admin" -ForegroundColor Red
        Write-Host "   الخطأ: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "15. اختبار طلبات Admin..." -ForegroundColor Yellow
    Write-Host "⚠️ لا يمكن اختبار طلبات Admin بدون Token" -ForegroundColor Yellow
}

Write-Host ""

Write-Host "========================================" -ForegroundColor Green
Write-Host "   ملخص اختبار جميع الـ APIs" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "   ✅ GET /dashboard - لوحة التحكم" -ForegroundColor Green
Write-Host "   ✅ POST /clients - إضافة عميل" -ForegroundColor Green
Write-Host "   ✅ GET /clients - جلب العملاء" -ForegroundColor Green
Write-Host "   ✅ POST /requests/contact - طلب تواصل Admin" -ForegroundColor Green
Write-Host ""
