...
Write-Host "   رفع التغييرات بدون إيموجي" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "test-apis.ps1 - script اختبار شامل" -ForegroundColor Green
Write-Host "push-apis-folder.ps1 - script رفع التغييرات" -ForegroundColor Green
Write-Host ""
...
try {
    $commitResult = & git commit -m "feat: إضافة مجلد الـ APIs منظم بدون إيموجي" 2>&1
    if ($LASTEXITCODE -eq 0) {
...
Write-Host "تم حذف جميع الإيموجي من الملفات" -ForegroundColor Green
...
Write-Host "   push-clean.ps1 - script رفع التغييرات" -ForegroundColor Gray
Write-Host ""
